...
const int CONTINUOUS_STOP_PULSE = 0;       // Giá trị PWM để dừng servo 360° (trung điểm)
...
void servo5Task(void *pvParameters)
{
  while (1)
  {
    if (isServo5Held)
    {
      ...
      vTaskDelay(pdMS_TO_TICKS(1000));         // Chờ 1.5 giây
...
void loop()
{
  ...
  if (isTestMode)
  {
    ...
    if (ps2x.ButtonPressed(PSB_L1))
    {
      ...
      pwm.setPWM(2, 0, SERVO_MAX_PULSE);
    }
