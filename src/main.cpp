...
Adafruit_PWMServoDriver pwm_mecanum = Adafruit_PWMServoDriver();
...
const float motor1Correction = 1;  // MT1 - right bottom
const float motor2Correction = 1;  // MT2 - right top
const float motor3Correction = 1;  // MT3 - left top
const float motor4Correction = -1; // MT4 - left bottom
...
// Speed levels (35–50)
int speedLevel = 0; // 0:30%, 1:50%
const float speedScales[] = {0.3, 0.5};
...
bool isRealseased = true; // Flag to track if servos are released
...
void mecanumDrivePWM(float vx, float vy, float omega, int speedLevel)
{
  ...
  // Nếu có lệnh xoay (omega != 0) và không di chuyển (vx, vy gần 0)
  if (abs(omega) > 0.15 && abs(vx) < 0.15 && abs(vy) < 0.15)
  {
    if (!isBraking)
    {
      setMotorSpeedSmooth(1, motor1Channels, 0);
      setMotorSpeedSmooth(2, motor2Channels, 0);
      setMotorSpeedSmooth(3, motor3Channels, 0);
      setMotorSpeedSmooth(4, motor4Channels, 0);
      isBraking = true;
      vTaskDelay(300 / portTICK_PERIOD_MS); // Đợi 100ms để đảm bảo phanh cứng
      Serial.println("Hard brake before rotation");
    }
  }
  else
  {
    isBraking = false; // Tắt trạng thái phanh khi không xoay
  }
...
void loop()
{
  ...
  if (!isTestMode)
  {
    ...
    {
      int speedLevelCount = sizeof(speedScales) / sizeof(speedScales[0]);
      speedLevel = (speedLevel + 1) % speedLevelCount; // Tăng + quay vòng
      volatileSpeedLevel = speedLevel;

      Serial.print("Speed switched to level ");
      Serial.print(speedLevel);
      Serial.print(" (");
      Serial.print(speedScales[speedLevel] * 100);
      Serial.println("%)");
    }
